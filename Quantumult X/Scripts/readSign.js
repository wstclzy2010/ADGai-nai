...
⚠️签到 首页时段宝箱 翻倍宝箱 账号信息 4处获取ck的地方
...
function videoSign(timeout=9000) {
  return new Promise((resolve) => {
   setTimeout( ()=>{
      let Url = {
...
function Msg() {
  let dd = ""
if ($.page.code == 0) {
dd +="【每日签到打卡】"+"今日打卡获🉐️"+$.page.data.todayAmount+"💰金币"+",已签到"+$.page.data.clockInDays+"天\n";

for(let i=0;i<$.page.data.dayList.length;i++)
 {
var tg=$.page.data.dayList[i].doneFlag==0?"(未签到)":"(已签到)";
if($.page.data.dayList[i].doneFlag!=undefined)
dd +="第"+$.page.data.dayList[i].day+"天签到"+",可获🉐️"+$.page.data.dayList[i].amount+"金币💰"+tg+"\n";

}
} 

if ($.treasure.code == 0)
dd+="【时段宝箱打卡】"+"✅获🉐️"+$.treasure.data.amount+"💰金币\n";
if ($.treasure.code == -1)
dd+="【时段宝箱打卡】"+$.treasure.msg+"\n";


if ($.video.code == 0)
dd+="【翻倍宝箱打卡】"+"✅获🉐️"+$.video.data.amount+"💰金币\n";
if ($.video.code == -1)
dd+="【翻倍宝箱打卡】"+$.video.msg+"\n";


if ($.watch.code == 0)
dd+="【看视频领金币】"+"✅获🉐️"+$.watch.data.amount+"💰金币\n";
if ($.watch.code == -1)
dd+="【看视频领金币】"+"明日再来\n";
if ($.watch.msg == "请稍候再试")
dd+="【看视频领金币】"+$.watch.msg+"\n";
...
if ($.fromGuid.code == 0)
dd+="【账号信息查询】"+"当前账号"+$.fromGuid.data.user.amount+"💰金币";
...
if ($request.url.indexOf("treasure_box") > -1) {
    $.setdata($request.url,'treasureurl')
        
    $.msg($.name,"","[获取时段宝箱数据]✅成功")}
